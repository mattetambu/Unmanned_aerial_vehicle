...
void* attitude_estimator_so3_comp_thread_main (void* args)
{
	...
	/* welcome user */
	fprintf (stdout, "Nonlinear SO3 Attitude Estimator started\n");
	fflush(stdout);
	...
	if (non_linear_SO3_AHRS_comp_parameters_init() != 0) {
	...
	while (!_shutdown_all_systems) {
		...
		if (updated) {
			...
			/* update parameters */
			non_linear_SO3_AHRS_comp_parameters_update();
		}
		...
		else
		{
			...
			/*if (!const_initialized && |check_out_of_bound (dt, , )) {
				const_initialized = (non_linear_SO3_AHRS_comp_parameters_update () == 0)? 1 : 0;
			}
	...
	if (orb_unadvertise (ORB_ID(vehicle_attitude), pub_att, pthread_self()) < 0)
		fprintf (stderr, "Attitude estimator thread failed to unadvertise the vehicle_attitude topic\n");

	if (orb_unsubscribe (ORB_ID(parameter_update), sub_params, pthread_self()) < 0)
		fprintf (stderr, "Attitude estimator thread failed to unsubscribe to parameter_update topic\n");

	if (orb_unsubscribe (ORB_ID(sensor_combined), sub_raw, pthread_self()) < 0)
		fprintf (stderr, "Attitude estimator thread failed to unsubscribe to sensor_combined topic\n");
