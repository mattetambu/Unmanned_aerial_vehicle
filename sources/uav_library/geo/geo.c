...
void map_projection_init(double lat_0, double lon_0) //lat_0, lon_0 are expected to be in correct format: -> 47.1234567 and not 471234567
{
...
{
	double lat_now_rad = lat_now / 180.0f * M_PI;
	double lon_now_rad = lon_now / 180.0f * M_PI;
	double lat_next_rad = lat_next / 180.0f * M_PI;
	double lon_next_rad = lon_next / 180.0f * M_PI;


	double d_lat = lat_next_rad - lat_now_rad;
	double d_lon = lon_next_rad - lon_now_rad;

	double a = sin(d_lat / 2.0f) * sin(d_lat / 2.0f) + sin(d_lon / 2.0f) * sin(d_lon / 2.0f) * cos(lat_now_rad) * cos(lat_next_rad);
...
// headed towards the end point.
	...
	int return_value = -1 /* ERROR */;	// Set error flag, cleared when valid result calculated.
	crosstrack_error->past_end = 0 /* false */;
	crosstrack_error->distance = 0.0f;
	crosstrack_error->bearing = 0.0f;

	// Return error if arguments are bad
	if (lat_now == 0.0f || lon_now == 0.0f || lat_start == 0.0f || lon_start == 0.0f || lat_end == 0.0f || lon_end == 0.0f) return return_value;
	...
	if (bearing_diff > M_PI_2 || bearing_diff < -M_PI_2) {
		crosstrack_error->past_end = 1 /* true */;
		return_value = 0 /* OK */;
		return return_value;
	}
	...
	if (sin(bearing_diff) >= 0) {
		crosstrack_error->bearing = _wrap_pi(bearing_track - M_PI_2);
		...
		float radius, float arc_start_bearing, float arc_sweep)
{
	...
	// Return error if arguments are bad
	if (lat_now == 0.0f || lon_now == 0.0f || lat_center == 0.0f || lon_center == 0.0f || radius == 0.0f) return return_value;
	...
	if (arc_sweep >= 0) {
		...
		if (bearing_sector_end > 2.0f * M_PI) bearing_sector_end -= M_TWOPI;

	} else {
		bearing_sector_end = arc_start_bearing;
		bearing_sector_start = arc_start_bearing - arc_sweep;

		if (bearing_sector_start < 0.0f) bearing_sector_start += M_TWOPI;
	}

	in_sector = 0 /* false */;

	// Case where sector does not span zero
	if (bearing_sector_end >= bearing_sector_start && bearing_now >= bearing_sector_start && bearing_now <= bearing_sector_end) in_sector = 1 /* true */;
	...
	if (in_sector) {
		...
		float lat_start = lat_now + start_disp_y * cos(lat_now) / 111111.0f;
		float lon_end = lon_now + end_disp_x / 111111.0f;
		float lat_end = lat_now + end_disp_y * cos(lat_now) / 111111.0f;
